...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // SOLO LOGIN - Sin lógica de registro
      ...
      // Si todo sale bien, entra al dashboard
      navigate('/dashboard');

    } catch (err) {
      // Mensaje genérico por seguridad
      setError('Credenciales no válidas. Acceso denegado.');
  ...
  return (
    <div className="min-h-screen bg-space-900 flex items-center justify-center p-4 font-sans relative overflow-hidden selection:bg-cyan-neon selection:text-space-950">
      ...
      <div className="glass-panel w-full max-w-md rounded-[2.5rem] overflow-hidden border border-white/10 animate-in fade-in zoom-in duration-500 relative z-10">
        ...
        {/* Header */}
        <div className="p-8 md:p-10 text-center relative pb-4">
          {/* Decorative Glow */}
        ...
        <form onSubmit={handleSubmit} className="p-8 pt-0 space-y-6" autoComplete="off">
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              ...
              <input
                ...
                placeholder="usuario@prometheus.com"
                value={email}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-cyan-neon hover:brightness-110 text-space-950 font-bold py-4 rounded-2xl transition-all shadow-[0_0_20px_rgba(0,217,255,0.3)] hover:shadow-[0_0_30px_rgba(0,217,255,0.5)] flex items-center justify-center gap-2 active:scale-[0.98] text-lg disabled:opacity-70 disabled:cursor-not-allowed mt-4"
          >
            {loading ? (
              <>
          ...
          <div className="text-center pt-4 border-t border-white/5 mt-4">
            <Link
              to="/"
              className="inline-flex items-center justify-center gap-2 text-slate-500 hover:text-cyan-neon font-medium transition-colors text-sm"
            >
              <Home size={16} />
